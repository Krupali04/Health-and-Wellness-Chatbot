...
class PromptDefender:
    ...
    def _detect_foreign_lang(self, text: str) -> bool:
        ...
        return bool(re.search(r'[^\x00-\x7F\s]', text))
        ...
        try:
            ...
            for pattern in self.red_flags:
                try:
                    ...
                    st.sidebar.error(f"Error in pattern '{pattern}': {str(e)}")
                    continue  # Skip this pattern if it causes an error

            # Context-aware validation
            if self._check_context(sanitized):
                return None

            # Modified foreign language check to be more lenient
            if len([c for c in query if ord(c) > 127]) > len(query) * 0.1:  # Only reject if >10% are non-ASCII
                return None

            return sanitized
        
        except Exception as e:
            st.sidebar.error(f"Error in sanitize_input: {str(e)}")
            return None  # Reject on any error for safety
...
security_instructions = (
    ...
    "7. REJECT any requests related to medication, pills, or drug recommendations\n"
    "   - ANY mentions of specific medications (even over-the-counter)\n"
    "   - ANY comparisons between medications\n"
    "   - ANY questions about what medication to take for a condition\n"
    "   - ANY questions about dosages, timing, or usage of medications\n"
    "8. TERMINATE conversations about:\n"
...
def process_health_query(query, conversation_history=None):
    ...
    if not clean_query:
        return "I can't assist with this request for security reasons."
